...
package FYF::FormBuilder;

=head1 NAME

FYF::FormBuilder - Easily generate and process stateful forms
...
sub new {
    ...
    unless ($self->{params} && ref $self->{params} ne 'HASH') {
        require CGI;
        $FYF::USE_PARAM_SEMICOLONS = 0;     # fuck ; in urls
        $self->{params} = CGI->new($self->{params});
    }

    # XXX not mod_perl safe (problem)
    $FYF::FormBuilder::Util::DEBUG = $self->{debug};
    ...
    if (lc($self->{messages}) eq 'auto') {
        ...
        $lang ||= 'default';
        $self->{messages} = FYF::FormBuilder::Messages->new(":$lang");
    } else {
        # ref or filename (::Messages will decode)
        $self->{messages} = FYF::FormBuilder::Messages->new($self->{messages});
    }
    ...
    for (@{$self->{fields}}) {
        ...
        if (ref $_ =~ /^FYF::FormBuilder::Field/i) {
...
sub new_field {
    ...
    my $f = FYF::FormBuilder::Field->new($self, %args);
...
sub header {
    ...
    return '' if $::TESTING;
    require CGI;
    return  FYF::header(%head);    # CGI.pm MOD_PERL fanciness
}
...
# must forcibly return scalar undef for FYF::Session easiness
sub sessionid {
    my $self = shift;
...
sub version {
    ...
    return '' if $::TESTING;
    if (ref $_[0]) {
        return "\n<!-- Generated by FYF::FormBuilder v$VERSION available from www.formbuilder.org -->\n";
    } else {
        return "FYF::FormBuilder v$VERSION by Nate Wiger. All Rights Reserved.\n";
    }
}
...
sub mailresults () {
    ...
    if (exists $args{plugin}) {
        my $lib = "FYF::FormBuilder::Mail::$args{plugin}";
        eval "use $lib";
...
sub AUTOLOAD {
    ...
    if ((! exists($self->{$name}) || @_) && ! $FYF::FormBuilder::Util::OURATTR{$name}) {
        if ($self->{fieldsubs}) {
            return $self->field(name => $name, @_);
        } else {
...
The goal of FYF::FormBuilder (B<FormBuilder>) is to provide an easy way
...
C<Text::Template>, C<Template Toolkit>, and C<FYF::FastTemplate>.
...
For the incredibly impatient, here's the quickest reference you can get:
    ...
    my $form = FYF::FormBuilder->new(
       ...
       # external source file (see FYF::FormBuilder::Source::File)
       source     => $file,
    );
...
For details on the external file format, see L<FYF::FormBuilder::Source::File>.
...
See L<FYF::FormBuilder::Messages> for more details.
...
For example:
    ...
    my $form = FYF::FormBuilder->new(... jsfunc => $jsfunc);
...
For details on this option, please refer to L<FYF::FormBuilder::Messages>.
...
However, you will want to specify this if you're using C<mod_perl>:
    ...
    sub handler {
        ...
        my $form = FYF::FormBuilder->new(... params => $r);
        print $form->render;
    }
...
are using a C<post> form method:
    ...
    my $q = new CGI;
    my $form = FYF::FormBuilder->new(... params => $q);
...
By default FYF::FormBuilder tries to be pretty smart for you, like
...
your form layout, which is pretty cool. See L<FYF::FormBuilder::Source::File>
...
For lots more information, please see L<FYF::FormBuilder::Template>.
...
form so that the user can change them if they wish. For example:
    ...
    my $form = FYF::FormBuilder->new(fields => \@fields,
                                     values => $rec);
...
Extending the above example:
    ...
    my $form = FYF::FormBuilder->new(template => 'some.tmpl');
...
See L<FYF::FormBuilder::Template> for more details.
...
easily integrate this with a session id module like C<FYF::Session>.
...
See L</"EXAMPLES"> for C<FYF::Session> example.
...
the namespace C<FYF::FormBuilder::Mail::*>.  These plugins may
...
See L<FYF::FormBuilder::Mail::FormatMultiPart> for more information.

This establishes a simple mail plugin implementation standard 
for your own mailresults() plugins.  The plugin should reside 
under the C<FYF::FormBuilder::Mail::*> namespace. It should have
...
C<FYF::FormBuilder::Util> and puke() if something goes wrong.

Calling $form->mailresults( plugin => 'Foo', ... ) will load
C<FYF::FormBuilder::Mail::Foo> and will pass the FormBuilder object
...
while C<field()> I<does> act "compliantly" for easy use in C<FYF::Session>,
...
This creates a session via C<FYF::Session>, and ties it in with B<FormBuilder>:
    ...
    # Initialize session
    my $session = FYF::Session->new('driver:File',
                                    $form->sessionid,
                                    { Directory=>'/tmp' });
...
Yes, it's pretty much that easy. See L<FYF::FormBuilder::Multi> for
...
option:
    ...
    my $form = FYF::FormBuilder->new(..., keepextras => 1);
...
even integrating it with modules like C<FYF::Application> if you want.
...
=head2 Why won't FYF::FormBuilder work with post requests?
...
Turn off sticky:
    ...
    my $form = FYF::FormBuilder->new(... sticky => 0);
...
Similarly, we can do the same thing with hashes:
    ...
    my $form = FYF::FormBuilder->new( ... validate => \%validate);
...
L<FYF::FormBuilder::Template>, L<FYF::FormBuilder::Messages>, 
L<FYF::FormBuilder::Multi>, L<FYF::FormBuilder::Source::File>,
L<FYF::FormBuilder::Field>, L<FYF::FormBuilder::Util>,
L<FYF::FormBuilder::Util>, L<HTML::Template>, L<Text::Template>
L<FYF::FastTemplate>
