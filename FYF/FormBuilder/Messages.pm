...
package FYF::FormBuilder::Messages;

=head1 NAME

FYF::FormBuilder::Messages - Localized message support for FormBuilder

=head1 SYNOPSIS

    use FYF::FormBuilder::Messages;

    my $mesg = FYF::FormBuilder::Messages->new(
                    $file || \%hash || ':locale'
               );
...
sub new {
    ...
    if (my $ref = ref $src) {
        ...
        require FYF::FormBuilder::Messages::default;
        %hash = FYF::FormBuilder::Messages::locale->messages;
...
Or, to use a specific locale, specify it as ":locale"
    ...
    my $form = FYF::FormBuilder->new(messages => ':da_DK');
...
Then, specify this file to C<new()>.
    ...
    my $form = FYF::FormBuilder->new(messages => 'messages.en');
