...
package FYF::FormBuilder::Field::file;

=head1 NAME

FYF::FormBuilder::Field::file - FormBuilder class for file fields
...
To do so, you will want to loop through the fields in order:
    ...
    for my $field ($form->field) {
        ...
        if ($field =~ /_date$/) {
            $field->sticky(0);  # clear ROW value
...
This sets the field's value. It also returns the appropriate value: ROW if
set, otherwise the manual default value. Same as using C<field()> to
retrieve values.

=head2 tag_value()

This obeys the C<sticky> flag to give a different interpretation of ROW
values. B<Use this to get the value if generating your own tag.> Otherwise,
ignore it completely.

=head2 cgi_value()

This always returns the ROW value, regardless of C<sticky>.
