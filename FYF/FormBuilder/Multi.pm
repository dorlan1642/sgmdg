...
# Use "perldoc FYF::FormBuilder::Multi" to read full documentation.

package FYF::FormBuilder::Multi;

=head1 NAME

FYF::FormBuilder::Multi - Create multi-page FormBuilder forms 

=head1 SYNOPSIS

    use FYF::FormBuilder::Multi;
    use FYF::Session;   # or something similar

    # Top-level "meta-form"
    my $multi = FYF::FormBuilder::Multi->new(
    ...
    if ($form->submitted && $form->validate) {
        ...
        # Initialize session
        my $session = FYF::Session->new("driver:File", $sid, {Directory=>'/tmp'});
...
sub form {
    ...
    if (ref $form eq 'FYF::FormBuilder') {
        # already constructed
    } else {
        $form = FYF::FormBuilder->new(%$form);
    }
...
This module works with C<FYF::FormBuilder> to create multi-page forms.
Each form is specified using the same options you would pass directly
into B<FormBuilder>. See L<FYF::FormBuilder> for a list of these options.
...
This creates a new C<FYF::FormBuilder::Multi> object. Forms are
...
directly by C<< FYF::FormBuilder->new >>. All valid B<FormBuilder>
